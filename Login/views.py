    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user.passwd != passwd:
                ...
                return redirect(reverse('HomePage', args=[userName]))
...
def passwdRecover(request):
    ...
    if request.method == 'POST':
        ...
        try:
            record = models.LoginCredentials.objects.filter(email=email)
            print(record[0].userName)
            print("emailfound")
        except models.LoginCredentials.unique_error_message:
            print("emailnotfound")
